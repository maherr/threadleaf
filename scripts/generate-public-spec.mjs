  ...
  for (const definition of definitions) {
    ...
    if (definition.observationPath) {
      ...
      fixture.observation = {
        ...
        status: text(observation.status, "observation status"),
...
function buildConformanceData(version, registry) {
  const claims = [
    ...
    },
    {
      ...
      limitations: [
        "The official Obsidian observation is explicitly unsupported and unverified; no pass is inferred without a recorded manual run.",
    ...
    },
    {
      ...
      fixtures: [],
      limitations: [
        "Unsupported tables, HTML, math, malformed syntax, and inline Live Preview processors remain source-visible.",
      ],
    },
    {
      id: "json-canvas",
  ...
  const gaps = [
    {
      id: "official-obsidian-roundtrip",
      title: "Official Obsidian round-trip observation",
      status: "not-verified",
      label: "informative",
      reason:
        "The synthetic Excalidraw corpus records an explicit manual protocol, but no official Obsidian run, result, screenshot, or generated bytes are retained.",
      source: sourceRef(excalidrawDefinition.observationPath),
    },
    {
