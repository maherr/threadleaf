...
function fail(message) {
  throw new CheckFailure(`[native-extension-build-artifact] ${message}`);
}
...
/** Sourcemaps are a build option, so absence is a configuration choice and not a broken build. */
async function readOptional(relativePath) {
  try {
    ...
    fail(
      `cannot read optional artifact ${relativePath}: ${error instanceof Error ? error.message : error}`,
    );
  }
}
...
async function main() {
  ...
  const requiredDistFiles = productionBundles.map((name) => `dist/main/${name}.cjs`);
  const optionalDistFiles = productionBundles.map((name) => `dist/main/${name}.cjs.map`);
  ...
  const artifacts = await Promise.all(
    [...requiredDistFiles, ...rendererAssets].map(async (relativePath) => [
      relativePath,
  ...
  const optionalArtifacts = (
    await Promise.all(
      optionalDistFiles.map(async (relativePath) => [
        relativePath,
        await readOptional(relativePath),
      ]),
    )
  ).filter(([, source]) => source !== null);
  artifacts.push(...optionalArtifacts);
  const skippedSourcemaps = optionalDistFiles.length - optionalArtifacts.length;
  ...
  const productionSdk =
    ...
    "";
  if (
    productionSdk.includes("options.entrypoint") ||
    productionSdk.includes("entrypoint: bundle")
  ) {
  ...
  console.log(
    `native extension build artifact gate passed over ${artifacts.length} shipped files, ${productionSources.length} production sources, and the built host surface${
      skippedSourcemaps === 0
        ? ""
        : ` (skipped ${skippedSourcemaps} absent sourcemap${skippedSourcemaps === 1 ? "" : "s"}; sourcemaps are off in this build)`
    }`,
  );
}
...
try {
  ...
  console.error(
    error instanceof CheckFailure
      ? error.message
      : `[native-extension-build-artifact] unexpected failure: ${error instanceof Error ? (error.stack ?? error.message) : error}`,
  );
}
