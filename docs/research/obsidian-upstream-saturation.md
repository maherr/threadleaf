...
- The spec page's content is schema-only; it contains no interaction/UI section. Obsidian's own Canvas
  *view* (drag/drop, grouping gestures, edit affordances) is therefore necessarily outside the open
  spec's scope by construction, not by an explicit disclaimer found on the page. Threadleaf may
  study, adopt, and build on those interaction ideas. Proprietary source code, assets, and extracted
  implementation details are not imported or published.

**Disposition**: **Depend** on the format and **Adapt** its interaction ideas. This is the cleanest
seam in the corpus: an MIT-licensed, stable, schema-only open
...
source code, extracted implementation details, and assets remain **Reject** for reuse.
...
| JSON Canvas | Depend (format); Adapt (interaction ideas); Reject (source/assets) | MIT, stable two years, schema-only, single first-party authority. Threadleaf may adopt and build on studied interaction ideas, but proprietary source code, extracted implementation details, and assets are not reused. |
