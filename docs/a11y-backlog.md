...
- **Reduced motion is enforced twice, deliberately.** A blanket `@media (prefers-reduced-motion:
  reduce)` rule (`styles.css:8502-8509`) zeroes `animation-duration`/`transition-duration` on
...
- **Focus visibility has a strong global fallback.** `button:focus-visible, a:focus-visible,
  ...
  outline by default (`styles.css:189-196`), on top of 34 component-specific `:focus-visible`
  ...
  (`.search-field:focus-within` at `styles.css:660`, `.graph-search-input:focus-within` at
  `:4121`, `.recovery-toolbar label:focus-within` at `:4699`, `.plugin-index-search
  label:has(input:focus-visible)` near `:6474`, `.plugin-search-field:has(input:focus-visible)`
  at `:6589`); see P2 finding 3 for the 2 that do not.
...
| 3 | P2 | Command palette and quick switcher search inputs | `src/renderer/styles.css:4999-5009` (`.palette-search input`) and `:3772-3782` (`.quick-switcher-search input`) | Both set `outline: 0` unconditionally with no compensating `:focus-within`/`:has()` rule on the wrapping `.palette-search`/`.quick-switcher-search` container, unlike every other search-input pattern in the app (`.search-field`, `.graph-search-input`, `.recovery-toolbar`, `.plugin-index-search`, `.plugin-search-field` all define one). Both dialogs auto-focus this input on open and it is the primary control of two of the app's most-used surfaces, so a keyboard user gets no visible confirmation the field is focused, on open or when tabbing back to it. | Add `.palette-search:focus-within` and `.quick-switcher-search:focus-within` rules mirroring `.search-field:focus-within` (`styles.css:660-663`). | 2.4.7 Focus Visible (AA) |
| 4 | P2 | Document view switch (Live / Source / Read / Plugin) | `src/renderer/styles.css:1448-1455` (`.document-view-switch button { min-width: 34px; min-height: 21px; }`) | The primary mode switcher for every open note is 21px tall, below the WCAG 2.2 24px minimum target size. | Raise `min-height` to at least 24px; 28-32px is more comfortable given the buttons already carry text labels. | 2.5.8 Target Size Minimum (AA) |
| 5 | P2 | Note tab close button | `src/renderer/styles.css:1367-1372` and `:1347-1351` (`.note-tab-close`) | `padding: 0` with no explicit `min-width`/`min-height`; the effective hit box is close to the 15px glyph's own rendered bounds, well under the 24px minimum, on the most frequently repeated per-row control in the tab strip. | Give `.note-tab-close` an explicit `min-width`/`min-height` of at least 24px; the visible glyph can stay small inside a larger padded box. | 2.5.8 Target Size Minimum (AA) |
...
| 9 | P3 | Renderer-wide CSS | `src/renderer/styles.css`, 112 occurrences of physical `left`/`right`/`margin-left`/`margin-right`/`padding-left`/`padding-right`/`text-align: left|right`/`float`, 0 occurrences of logical equivalents (`inset-inline-*`, `margin-inline-*`, `text-align: start|end`) | Not a defect today: the UI ships English-only, `<html>` has no `dir` toggle, and there is no RTL locale switch. It is debt that would need a systematic pass before any RTL UI localization is attempted. Distinct from finding 2, which is about bidi handling of arbitrary user *content*, not the direction of the UI chrome itself. | Track as forward-looking debt only; do not schedule unless RTL UI localization is actually planned. | No applicable SC; localization readiness note |
