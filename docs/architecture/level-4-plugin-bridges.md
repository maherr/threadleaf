...
- controller version, controller executable SHA-256, trusted-controller-manifest ID and SHA-256, evidence-harness version and tree SHA-256, issuer key ID and key-identity SHA-256, issuer-trust-store version and identity SHA-256, platform, architecture, and Electron version;
...
The build manifest is a sorted, path-normalized inventory of the executable and resource closure of the installed package. Hashing only `package.json`, a semantic version, a Git commit, or one bundle is insufficient. The relevant `dist` manifest separately names the main, preload, main-renderer, plugin-renderer, runtime, and generated compatibility-registry bundles so a dirty or partially rebuilt installation cannot reuse a receipt from another effective build.
...
2. recompute the supplied current controller executable and evidence-harness tree, require them to match the current trusted controller manifest, and then require the receipt's controller version, controller executable hash, controller-manifest identity, harness version, and harness-tree hash to match those current trusted values, not merely the artifacts the receipt names;
...
The exact verification tuple is the canonical digest of the effective build identity, exact package identity, authority-profile digest, workflow-definition hash, fixture-tree hash, platform, architecture, controller executable hash, trusted-controller-manifest hash, evidence-harness version and tree hash, issuer key identity, and issuer-trust-store version and identity. The replay index records that tuple digest beside the nonce, run ID, and receipt-file hash.
...
2. Define the canonical v2 payload/envelope schemas, co-privileged receipt threat model, dedicated controller/finalizer, checked-in trust policy, trusted controller manifest, signed attempt-record split, and no-replace atomic publication protocol.
3. Implement the dedicated verifier, exact packaged-app and relevant-`dist` build manifest, fixture/package tree hashing, append-only nonce replay index, current-controller and current-harness comparison, full verification tuple, and fail-closed local-artifact comparison.
...
5. Update `scripts/generate-plugin-compatibility-registry.mjs` and `src/shared/plugins.ts` for exact identity, effective build identity, named workflows, platform scope, completion assertions, reload assertions, controller-manifest identity, harness identity, issuer-key identity, trust-store identity, and limitations.
