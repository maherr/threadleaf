...
from a local Unicode Character Database and committed as `src/generated/case-folding-table.ts`, so
the runtime never fetches Unicode data over the network. Hosts without the source database verify
the exact committed artifact digest; regeneration still requires the pinned local source. Only
Common and Simple status mappings are applied. Full mappings that grow a string in length, such as
sharp s to "ss", and Turkic dotted-I
