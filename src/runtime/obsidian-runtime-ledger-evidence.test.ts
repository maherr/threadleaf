...
describe("Obsidian 1.13.7 runtime ledger evidence", () => {
  ...
  it('proves modal, notice, suggest-modal, and keymap lifecycles through require("obsidian")', async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(pluginPath, "main.js"),
        [
          'const { Keymap, Modal, Notice, Platform, Plugin, Scope, SuggestModal } = require("obsidian");',
          ...
          '    const modEvent = new document.defaultView.KeyboardEvent("keydown", { key: "k", ...modKey });',
          '    const splitEvent = new document.defaultView.KeyboardEvent("keydown", { key: "k", ...modKey, altKey: true });',
          '    const windowEvent = new document.defaultView.KeyboardEvent("keydown", { key: "k", ...modKey, altKey: true, shiftKey: true });',
    ...
    await withTestDocument(async () => {
      ...
      try {
        ...
        await fs.writeFile(
          path.join(pluginPath, "main.js"),
          [
            'const { FileSystemAdapter, Plugin } = require("obsidian");',
            "class LedgerPlugin extends Plugin {",
            "  async onload() {",
            ...
            "    const adapterLocalBytes = await FileSystemAdapter.readLocalFile(adapterFilePath);",
            ...
            '    await FileSystemAdapter.mkdir(adapter.getFullPath("Static folder"));',
            '    await adapter.write("Remove.me", "remove");',
            '    await adapter.remove("Remove.me");',
            '    await adapter.rmdir("Adapter folder", false);',
            '    const adapterSystemTrashMissing = await adapter.trashSystem("Missing.system-trash");',
            ...
            "      attachmentPath,",
            "      prompted,",
            '      adapter: { name: adapterName, basePath: adapterBasePath, fullPath: adapterFullPath, read: adapterRead, readBinary: bytesEqual(adapterReadBinary, [3, 4]), written: adapterWritten, processed: adapterProcessed, resourcePath: adapterResourcePath, filePath: adapterFilePath, localBytes: bytesEqual(adapterLocalBytes, [65, 68, 65, 80, 84, 69, 82, 45, 65, 80, 80, 69, 78, 68]), exists: adapterExists, stat: adapterStat, listed: adapterList, staticFolder: await adapter.exists("Static folder"), removed: (await adapter.exists("Remove.me")) === false, removedFolder: (await adapter.exists("Adapter folder")) === false, systemTrashMissing: adapterSystemTrashMissing, copiedBinary: bytesEqual(adapterCopiedBinary, [7, 8, 9]), renamedTrashed: (await adapter.exists("Adapter renamed.md")) === false && (await adapter.exists(".trash/Adapter renamed.md")) === true },',
  ...
  /** @compatibility-test-id obsidian-runtime.workspace-hierarchy.v1 */
  ...
  it('proves the core events, file identity, vault, and metadata bindings through require("obsidian")', async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(pluginPath, "main.js"),
        [
          'const { Editor, Events, Plugin, TAbstractFile, TFile, TFolder, Vault, WorkspaceContainer, WorkspaceMobileDrawer, WorkspaceParent, WorkspaceRibbon, WorkspaceRoot, WorkspaceSidedock, WorkspaceSplit, WorkspaceWindow } = require("obsidian");',
          "class LedgerPlugin extends Plugin {",
          "  async onload() {",
          "    const vault = this.app.vault;",
          ...
          "    leftSplit.collapse();",
          "    const leftCollapsed = leftSplit.collapsed;",
          "    leftSplit.expand();",
          "    leftSplit.toggle();",
          "    const leftToggled = leftSplit.collapsed;",
          "    leftSplit.expand();",
          "    const drawer = new WorkspaceMobileDrawer();",
          "    drawer.collapse();",
          "    const drawerCollapsed = drawer.collapsed;",
          "    drawer.expand();",
          "    drawer.toggle();",
          "    const drawerToggled = drawer.collapsed;",
          "    drawer.expand();",
          '    const hierarchyWindow = new WorkspaceWindow(workspace, "horizontal");',
          ...
          "        layoutCalls,",
          "        hierarchy: {",
          "          leftSplitIsSideDock: leftSplit instanceof WorkspaceSidedock,",
          "          rightSplitIsSideDock: workspace.rightSplit instanceof WorkspaceSidedock,",
          "          leftRibbonIsRibbon: workspace.leftRibbon instanceof WorkspaceRibbon,",
          "          rightRibbonIsRibbon: workspace.rightRibbon instanceof WorkspaceRibbon,",
          "          rootIsRoot: workspace.rootSplit instanceof WorkspaceRoot,",
          "          rootIsContainer: workspace.rootSplit instanceof WorkspaceContainer,",
          "          windowIsWindow: hierarchyWindow instanceof WorkspaceWindow,",
          "          drawerParentIsParent: drawer.parent instanceof WorkspaceParent,",
          '          rootWindowObjects: typeof workspace.rootSplit.win === "object" && typeof workspace.rootSplit.doc === "object",',
          '          windowObjects: typeof hierarchyWindow.win === "object" && typeof hierarchyWindow.doc === "object",',
          "          leftCollapsed,",
          "          leftToggled,",
          "          drawerCollapsed,",
          "          drawerToggled,",
      ...
      try {
        ...
        expect(
          ...
          workspace: {
            ...
            layoutCalls: ["layout"],
            hierarchy: {
              leftSplitIsSideDock: true,
              rightSplitIsSideDock: true,
              leftRibbonIsRibbon: true,
              rightRibbonIsRibbon: true,
              rootIsRoot: true,
              rootIsContainer: true,
              windowIsWindow: true,
              drawerParentIsParent: true,
              rootWindowObjects: true,
              windowObjects: true,
              leftCollapsed: true,
              leftToggled: true,
  ...
  it('proves DOM-backed settings components through require("obsidian")', async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(pluginPath, "main.js"),
        [
          'const { AbstractTextComponent, ButtonComponent, ColorComponent, DisplayValueComponent, DropdownComponent, ExtraButtonComponent, MomentFormatComponent, Plugin, ProgressBarComponent, SearchComponent, Setting, SettingGroup, SettingPage, SliderComponent, TextAreaComponent, TextComponent, ToggleComponent, ValueComponent } = require("obsidian");',
          "class LedgerPlugin extends Plugin {",
          "  async onload() {",
          '    const root = document.createElement("main");',
          "    document.body.append(root);",
          '    const settingGroup = new SettingGroup(root).setHeading("Group heading").addClass("fixture-group");',
          "    let groupSetting, groupSearch, groupExtra;",
          "    const groupSearchChanges = [];",
          '    settingGroup.addSetting((component) => { groupSetting = component.setName("Grouped"); });',
          '    settingGroup.addSearch((component) => { groupSearch = component.onChange((value) => groupSearchChanges.push(value)).setValue("query"); component.onChanged(); });',
          '    settingGroup.addExtraButton((component) => { groupExtra = component.setIcon("x"); });',
          "    class LedgerPage extends SettingPage {",
          '      display() { this.containerEl.textContent = "Page body"; }',
          "    }",
          '    const page = new LedgerPage(); page.title = "Ledger page"; page.titlebarEl.textContent = page.title; page.display();',
          "    const pageBeforeHide = { rootClass: page.rootEl.className, title: page.titlebarEl.textContent, body: page.containerEl.textContent };",
          "    page.hide();",
          "    const pageAfterHide = page.containerEl.childElementCount;",
          "    const textChanges = [];",
          ...
          '    const valueComponent = new TextComponent(directRoot).setValue("option");',
          '    const valueComponentListenerResult = valueComponent.registerOptionListener({ option: (value) => value || "fallback" }, "option");',
          ...
          "      groupIsGroup: settingGroup instanceof SettingGroup,",
          "      groupClass: settingGroup.listEl.className,",
          "      groupHeading: settingGroup.listEl.firstElementChild && settingGroup.listEl.firstElementChild.textContent,",
          "      groupSettingName: groupSetting.nameEl.textContent,",
          "      groupSearchValue: groupSearch.getValue(),",
          "      groupSearchChanges,",
          "      groupExtraIsExtra: groupExtra instanceof ExtraButtonComponent,",
          "      pageBeforeHide,",
          "      pageAfterHide,",
          "      valueComponentIsValueComponent: valueComponent instanceof ValueComponent,",
          "      valueComponentChain: valueComponentListenerResult === valueComponent,",
          '      valueComponentValue: valueComponent.setValue("updated").getValue(),',
        ...
        try {
          ...
          expect(
            ...
            beforeClear: {
              ...
              groupIsGroup: true,
              groupClass: "setting-group fixture-group",
              groupHeading: "Group heading",
              groupSettingName: "Grouped",
              groupSearchValue: "query",
              groupSearchChanges: ["query"],
              groupExtraIsExtra: true,
              pageBeforeHide: {
                rootClass: "setting-page",
                title: "Ledger page",
                body: "Page body",
              },
              pageAfterHide: 0,
              valueComponentIsValueComponent: true,
              valueComponentChain: true,
              valueComponentValue: "updated",
              textIsText: true,
              toggleIsToggle: true,
