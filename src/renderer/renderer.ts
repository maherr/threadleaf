...
import { BaseViewController } from "./base-view";
...
const elements = {
  ...
  editorControls: getElement("editor-controls"),
  ...
  attachmentMoveCurrentLabel: getElement("attachment-move-current-label"),
  ...
  attachmentInsertSubmit: getButton("attachment-insert-submit"),
  attachmentInsertError: getElement("attachment-insert-error"),
  attachmentInsertPreviewMessage: getElement("attachment-insert-preview-message"),
  attachmentInsertProof: getElement("attachment-insert-proof"),
  attachmentInsertProofSummary: getElement("attachment-insert-proof-summary"),
  attachmentInsertProofList: getElement("attachment-insert-proof-list"),
  attachmentInsertVault: getElement("attachment-insert-vault"),
...
function paneElementsFor(
  ...
  return {
    ...
    editorControls: element("editor-controls"),
...
const baseViews = new Map<WorkspacePaneId, BaseViewController>();
for (const [paneId, pane] of paneElements) {
  baseViews.set(
    paneId,
    new BaseViewController(pane.baseView, {
      openPath: async (path) => {
        await openNote(path, undefined, paneId);
      },
    }),
...
function activeWorkspaceDocumentPath(
  ...
  return (
    pane?.activeBase?.path ??
    pane?.activeCanvas?.path ??
    pane?.activePluginFile?.path ??
    pane?.activeNote?.path ??
    null
  );
}
  ...
  return [
    ...
    EditorView.domEventHandlers({
      ...
      dragleave: (_event, view) => {
        setEditorAttachmentDropActive(view, false);
        return false;
      },
      dragend: (_event, view) => {
        setEditorAttachmentDropActive(view, false);
      ...
      drop: (event, view) => handleEditorAttachmentDrop(paneId, view, event),
      paste: (event, view) => handleEditorAttachmentPaste(paneId, view, event),
    }),
...
function navigatorEntryIcon(
  ...
  if (entry.kind === "base") {
    return interfaceIcon(FileSpreadsheet, "navigator-tree-entry-icon");
  }
...
function renderDocumentView(): void {
  ...
  const activeBase = activePane?.activeBase;
  ...
  const hasBase = activeBase !== undefined && activeBase !== null;
  ...
  elements.workspaceRoot.dataset.activeDocumentKind = hasBase ? "base" : "standard";
  elements.editorControls.hidden = hasBase || hasCanvas;
  ...
  elements.noteEmpty.hidden = hasNote || hasBase || hasCanvas || hasPluginFile || plugin;
  ...
  renderDocumentViewToolbarLabel(elements.notePath, {
    loadedPath:
      loadedNote?.path ?? activeBase?.path ?? activeCanvas?.path ?? activePluginFile?.path ?? null,
    unavailable: activeUnavailable,
    pluginLabel,
  });
...
async function openAttachmentRelinkDialog(
  sourceNotePath: string,
  missingTarget: string,
  missingPath: string,
  sourceNoteRevision: string,
  ...
  if (!loadedVaultId || readOnlyVault() || busy) {
    showToast(
      readOnlyVault()
        ? "Open a local vault before relinking missing attachments."
        : "Threadleaf is finishing another action.",
    );
    return;
  }
  ...
  attachmentMoveBusy = false;
  attachmentMoveVaultId = loadedVaultId;
  attachmentMoveSourcePath = missingPath;
  attachmentMoveRevision = sourceNoteRevision;
  attachmentMoveOperation = "relink";
  attachmentRelinkSourceNotePath = sourceNotePath;
  attachmentRelinkMissingTarget = missingTarget;
  attachmentMoveBlockers = [];
  attachmentMoveRewrites = [];
  attachmentMoveConfirmationId = null;
  elements.attachmentMoveTarget.value = "";
  elements.attachmentMoveError.textContent = "";
  elements.attachmentMovePreviewMessage.textContent = "";
  elements.attachmentMoveDialog.showModal();
  renderAttachmentMoveDialog();
...
function setEditorAttachmentDropActive(view: EditorView, active: boolean): void {
  view.dom.dataset.attachmentDropActive = String(active);
}

function editorAttachmentSource(
  paneId: WorkspacePaneId,
  view: EditorView,
): {
  vaultId: string;
...
function editorAttachmentCanStage(paneId: WorkspacePaneId, view: EditorView): boolean {
  return (
    editorAttachmentSource(paneId, view) !== null &&
    !busy &&
...
function handleEditorAttachmentDrag(
  paneId: WorkspacePaneId,
  view: EditorView,
  event: DragEvent,
): boolean {
...
function handleEditorAttachmentDrop(
  paneId: WorkspacePaneId,
  view: EditorView,
  event: DragEvent,
): boolean {
  ...
  if (!editorAttachmentCanStage(paneId, view)) {
    showToast(
      "Open one writable Markdown note and finish the current action before inserting a file.",
    );
    return true;
  }
...
function handleEditorAttachmentPaste(
  paneId: WorkspacePaneId,
  view: EditorView,
...
async function stageEditorAttachmentInsert(
  ...
  if (staged.status === "too-large") {
    showToast(
      staged.phase === "declared"
        ? `That file is ${formatByteCount(staged.byteLength)}. Attachment insertion is limited to ${formatByteCount(MAX_VAULT_ATTACHMENT_BYTES)}.`
        : `That file exceeds the ${formatByteCount(MAX_VAULT_ATTACHMENT_BYTES)} attachment insertion limit.`,
    );
    return;
  }
  if (staged.status === "unreadable") {
    showToast("Threadleaf could not read that file. No vault bytes were staged.");
    return;
  }
  if (!isSafeExternalAttachmentTarget(staged.sourceFileName)) {
    showToast(
      "Insert a supported raster image or passive attachment file. SVG, HTML, Markdown, and Canvas files are not accepted here.",
    );
    return;
  }
  if (paneId !== activePaneContextId) {
  ...
  if (
    ...
    showToast("The editor changed before the selected file was staged. Drop or paste it again.");
    return;
  }
  if (!(await tryFlushAllPaneAutosaves("note-mutation"))) {
    showToast("Autosave could not finish, so Threadleaf did not open an insertion review.");
    return;
  }
  ...
  window.requestAnimationFrame(() => {
    const nameStart = elements.attachmentInsertTarget.value.lastIndexOf("/") + 1;
    elements.attachmentInsertTarget.focus();
    elements.attachmentInsertTarget.setSelectionRange(
      nameStart,
      elements.attachmentInsertTarget.value.length,
    );
  });
}
...
function renderAttachmentInsertDialog(): void {
  ...
  elements.attachmentInsertVault.textContent = staleVault
    ...
    : currentSnapshot
      ...
      : "Active vault";
  const error = elements.attachmentInsertError.textContent ?? "";
  elements.attachmentInsertError.hidden = error.length === 0;
  ...
  const origin = document.createElement("span");
  origin.className = "move-note-blocker-origin";
  origin.textContent = `${attachmentInsertPreview.sourceFileName} · ${formatByteCount(attachmentInsertPreview.byteLength)}`;
  origin.title = `SHA-256 ${attachmentInsertPreview.contentRevision}`;
  const change = document.createElement("span");
  change.className = "move-note-blocker-change";
  const target = document.createElement("span");
  target.textContent = attachmentInsertPreview.targetPath;
  const arrow = document.createElement("span");
  arrow.className = "move-note-blocker-arrow";
  arrow.ariaHidden = "true";
  arrow.textContent = "→";
  const reference = document.createElement("code");
  reference.textContent = attachmentInsertPreview.referenceText;
  change.append(target, arrow, reference);
  item.append(origin, change);
  elements.attachmentInsertProofList.append(item);
}
...
function focusAttachmentInsertResult(
  paneId: WorkspacePaneId,
  path: string,
  selectionAfter: number,
): void {
  runInPaneContext(paneId, () => {
    if (!loadedNote || loadedNote.path !== path) return;
    const position = Math.max(0, Math.min(selectionAfter, editor.state.doc.length));
    editor.dispatch({ selection: { anchor: position }, scrollIntoView: true });
    editor.focus();
  });
}

async function insertCurrentExternalAttachment(): Promise<void> {
  const pending = pendingAttachmentInsert;
  if (!pending || attachmentInsertBusy) return;
  const targetPath = elements.attachmentInsertTarget.value.trim();
  if (!targetPath) {
    elements.attachmentInsertError.textContent = "Enter a vault-relative target file path.";
    renderAttachmentInsertDialog();
    return;
  }
  if (!(await tryFlushAllPaneAutosaves("note-mutation"))) {
    elements.attachmentInsertError.textContent =
      "Autosave could not finish. Threadleaf did not start the attachment insertion.";
    renderAttachmentInsertDialog();
    return;
  }
  if (currentSnapshot?.vault.id !== pending.vaultId) {
    elements.attachmentInsertError.textContent =
      "The vault changed. Cancel this review and drop or paste the file again.";
    pendingAttachmentInsert = null;
    attachmentInsertPreview = null;
    attachmentInsertConfirmationId = null;
    renderAttachmentInsertDialog();
  ...
  let response: AttachmentInsertResponse | null = null;
  let completed: {
    paneId: WorkspacePaneId;
    path: string;
    selectionAfter: number;
  ...
  const submittedConfirmationId = attachmentInsertConfirmationId;
  ...
  setActionState(true);
  try {
    response = await window.threadleaf.insertAttachment(
    ...
    if (response.outcome.status === "requires-confirmation") {
      attachmentInsertPreview = response.outcome.preview;
      attachmentInsertConfirmationId = response.outcome.confirmationId;
      elements.attachmentInsertPreviewMessage.textContent = submittedConfirmationId
        ? "The insertion plan changed. Review the refreshed byte, path, selection, and reference proof, then confirm again."
        : "Review the exact byte identity, target path, and generated editor reference below. Submit again to commit both parts.";
    } else if (
      ...
      completed = {
        ...
        message: affectedElsewhere
          ? `The insertion completed in previously active vault "${response.affectedVaultName ?? "previous vault"}".`
          : outcome.status === "committed"
            ? `Inserted ${outcome.attachmentPath} and added one reference.`
            : outcome.message,
      };
    } else {
      attachmentInsertPreview = null;
      ...
      elements.attachmentInsertError.textContent =
        response.outcome.status === "manual-conflict" && response.affectedVaultName
          ? `The insertion reached manual review in previously active vault "${response.affectedVaultName}". ${response.outcome.message}`
...
async function activatePreviewAttachmentAction(actionButton: HTMLButtonElement): Promise<void> {
  ...
  if (action === "relink" || action === "restore" || action === "paste") {
    ...
    const sourceNotePath =
      actionButton.dataset.threadleafAttachmentSourceNotePath ??
      card?.dataset.threadleafAttachmentSourceNotePath;
    if (!sourceNoteRevision || !missingTarget || !sourceNotePath) {
      showToast(
        "This missing attachment card is no longer current. Refresh Reading view and try again.",
      );
      return;
...
async function relinkCurrentAttachment(): Promise<void> {
  const expectedVaultId = attachmentMoveVaultId;
  const sourceNotePath = attachmentRelinkSourceNotePath;
  const missingTarget = attachmentRelinkMissingTarget;
  const expectedSourceRevision = attachmentMoveRevision;
  if (
    !expectedVaultId ||
    !sourceNotePath ||
  ...
  if (!requestedPath) {
    elements.attachmentMoveError.textContent = "Enter an existing vault-relative attachment path.";
    attachmentMoveBlockers = [];
    attachmentMoveRewrites = [];
    attachmentMoveConfirmationId = null;
    elements.attachmentMovePreviewMessage.textContent = "";
  ...
  if (!(await tryFlushAllPaneAutosaves("note-mutation"))) {
    elements.attachmentMoveError.textContent =
      "Autosave could not finish. Threadleaf did not start relinking the attachment.";
    renderAttachmentMoveDialog();
    return;
  }
  if (currentSnapshot?.vault.id !== expectedVaultId) {
    elements.attachmentMoveError.textContent =
      "The vault changed. Cancel and reopen Relink from the missing attachment card.";
    attachmentMoveBlockers = [];
    attachmentMoveRewrites = [];
    attachmentMoveConfirmationId = null;
    elements.attachmentMovePreviewMessage.textContent = "";
  ...
  let response: AttachmentRelinkResponse | null = null;
  let committedReplacementPath: string | null = null;
  let committedVaultNotice: string | null = null;
  const submittedConfirmationId = attachmentMoveConfirmationId;
  attachmentMoveBusy = true;
  attachmentMoveBlockers = [];
  ...
  setActionState(true);
  try {
    response = await window.threadleaf.relinkAttachment(
      sourceNotePath,
      missingTarget,
      requestedPath,
      expectedSourceRevision,
    ...
    render(response.snapshot);
    if (response.outcome.status === "requires-confirmation") {
      attachmentMoveRewrites = [response.outcome.rewrite];
      attachmentMoveConfirmationId = response.outcome.confirmationId;
      elements.attachmentMovePreviewMessage.textContent = submittedConfirmationId
        ? "The relink plan changed on disk. Review this refreshed exact target change, then confirm it again."
        : "Review the exact missing target replacement below. Submit again to rewrite the source note as one recoverable operation.";
    } else if (response.outcome.status === "committed") {
      committedReplacementPath = response.outcome.rewrite.replacementPath;
      committedVaultNotice = attachmentMoveCommitNotice(response, "committed");
    } else {
      attachmentMoveConfirmationId = null;
      attachmentMoveRewrites = [];
      elements.attachmentMovePreviewMessage.textContent = "";
      elements.attachmentMoveError.textContent = response.outcome.message;
    }
  } catch {
    elements.attachmentMoveError.textContent =
      "The attachment relink could not be confirmed. Review the current source note and attachment; Threadleaf made no further changes.";
  } finally {
    attachmentMoveBusy = false;
    setActionState(false);
    if (elements.attachmentMoveDialog.open) renderAttachmentMoveDialog();
  ...
  if (committedReplacementPath) {
    const operationNotice = `Relinked the missing attachment to ${committedReplacementPath}.`;
    closeAttachmentMoveDialog(false);
    showToast(
      committedVaultNotice ? `${committedVaultNotice} ${operationNotice}` : operationNotice,
    );
...
async function moveCurrentAttachment(): Promise<void> {
  ...
  const renaming = operation === "rename";
  const expectedVaultId = attachmentMoveVaultId;
...
function renderWorkspacePanes(
  ...
  for (const paneId of workspacePaneIdsInRenderOrder(activePaneId)) {
    ...
    if (pane.activeBase) {
      ...
      paneUi.baseView.hidden = false;
      paneUi.notePath.textContent = pane.activeBase.path;
      baseViews.get(paneId)?.render(pane.activeBase);
  ...
  if (activePane?.activeBase) {
    renderNote(null);
    elements.noteEmpty.hidden = true;
    elements.noteView.hidden = true;
    elements.canvasView.hidden = true;
    elements.baseView.hidden = false;
  } else if (activePane?.activeCanvas) {
    renderNote(null);
    elements.noteEmpty.hidden = true;
    elements.noteView.hidden = true;
    elements.baseView.hidden = true;
...
async function openNote(
  ...
  return (
    ...
    workspacePaneSnapshot(paneId)?.activeBase?.path === filePath ||
...
elements.attachmentInsertDialog.addEventListener("cancel", (event) => {
  event.preventDefault();
  closeAttachmentInsertDialog();
});
elements.attachmentInsertDialog.addEventListener("click", (event) => {
  if (event.target === elements.attachmentInsertDialog) closeAttachmentInsertDialog();
});
